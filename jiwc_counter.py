...
class JIWC_Counter:
	...
	def count_categories(self, tok_list):
		#put check for the dict-b file
		categ_out = {a:[{}, 0] for a in self.categs.keys()}
		df=None
		try:
			df = pd.read_csv(self.args.dict)
		except Exception as e:
			print('Error occured when reading JIWC-B csv file. Program terminating.')
			print(e)
			exit()
		...
		if self.args.print_categ is not None and len(self.args.print_categ)>0:
			...
			for m in self.args.print_categ:
				...
				if m.lower()[:3] in categ_eng.keys():
					...
					# print('Total {0} word types : {1}'.format(categ, len(allwords_)))
					print('{0}/{1} words: '.format(categ, self.categs[categ]), end=' ')
...
def main():
	...
	arg_parser.add_argument('--dict', type=str, help='path of the JIWC-B dictionary to use')
